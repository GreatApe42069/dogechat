...
struct PublicTimelineStore {
    ...
    private var geohashTimelines: [String: [DogechatMessage]] = [:]
    ...
    mutating func append(_ message: DogechatMessage, to channel: ChannelID) {
        switch channel {
        case .mesh:
    ...
    mutating func append(_ message: DogechatMessage, toGeohash geohash: String) {
    ...
    mutating func appendIfAbsent(_ message: DogechatMessage, toGeohash geohash: String) -> Bool {
        var timeline = geohashTimelines[geohash] ?? []
    ...
    mutating func messages(for channel: ChannelID) -> [DogechatMessage] {
        switch channel {
        case .mesh:
            return meshTimeline
        case .location(let channel):
    ...
    @discardableResult
    mutating func removeMessage(withID id: String) -> DogechatMessage? {
    ...
    mutating func removeMessages(in geohash: String, where predicate: (DogechatMessage) -> Bool) {
        var timeline = geohashTimelines[geohash] ?? []
        timeline.removeAll(where: predicate)
        geohashTimelines[geohash] = timeline.isEmpty ? nil : timeline
    }

    mutating func mutateGeohash(_ geohash: String, _ transform: (inout [DogechatMessage]) -> Void) {
        var timeline = geohashTimelines[geohash] ?? []
        transform(&timeline)
        geohashTimelines[geohash] = timeline.isEmpty ? nil : timeline
    }
    ...
    private func trimGeohashTimelineIfNeeded(_ timeline: inout [DogechatMessage]) {
