...
final class ChatViewModel: ObservableObject, DogechatDelegate, CommandContextProvider, GeohashParticipantContext, MessageFormattingContext {
    ...
    func blockGeohashUser(pubkeyHexLowercased: String, displayName: String) {
        ...
        if let gh = currentGeohash {
            let predicate: (DogechatMessage) -> Bool = { [self] msg in
    ...
    @MainActor
    func isSelfMessage(_ message: DogechatMessage) -> Bool {
        if let spid = message.senderPeerID {
            // In geohash channels, compare against our per-geohash nostr short ID
            if case .location(let ch) = activeChannel, spid.isGeoChat {
                let myGeo: NostrIdentity? = {
    ...
    @MainActor
    func senderColor(for message: DogechatMessage, isDark: Bool) -> Color {
        return peerColor(for: message, isDark: isDark)
    }

    @MainActor
    func peerURL(for peerID: PeerID) -> URL? {
        return URL(string: "dogechat://user/\(peerID.toPercentEncoded())")
    }
        ...
        DispatchQueue.main.async {
            ...
            if meshPeerSet.isEmpty {
                ...
                if !newPeers.isEmpty {
                    ...
                    SecureLogger.info(
                        "👥 Sent dogechatters nearby notification for \(meshPeers.count) mesh peers (new: \(newPeers.count))",
                        category: .session
                    )
    ...
    @MainActor
    func handlePublicMessage(_ message: DogechatMessage) {
        ...
        func checkForMentions(_ message: DogechatMessage) {    // Determine our acceptable mention token. If any connected peer shares our nickname,
    ...
    func sendHapticFeedback(for message: DogechatMessage) {        #if os(iOS)
...
extension ChatViewModel: PublicMessagePipelineDelegate {
    func pipelineCurrentMessages(_ pipeline: PublicMessagePipeline) -> [DogechatMessage] {
        messages
    }

    func pipeline(_ pipeline: PublicMessagePipeline, setMessages messages: [DogechatMessage]) {
    ...
    func pipelinePrewarmMessage(_ pipeline: PublicMessagePipeline, message: DogechatMessage) {
