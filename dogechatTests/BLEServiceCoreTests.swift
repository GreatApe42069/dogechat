...
private func makePublicPacket(content: String, sender: PeerID, timestamp: UInt64) -> DogechatPacket {
    DogechatPacket(
...
private final class PublicCaptureDelegate: DogechatDelegate {
    private let lock = NSLock()
    private(set) var publicMessages: [DogechatMessage] = []
    ...
    func publicMessagesSnapshot() -> [DogechatMessage] {
        lock.lock()
        defer { lock.unlock() }
