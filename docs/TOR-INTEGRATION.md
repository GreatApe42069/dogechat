...
- TorManager
  ...
  - Fails closed by default until Tor is bootstrapped. For local development only, define DOGECHAT_DEV_ALLOW_CLEARNET to bypass Tor.
- TorURLSession
...
torrc template
The generated torrc (under Application Support/dogechat/tor/torrc) is:

  DataDirectory <AppSupport>/dogechat/tor
  ClientOnly 1
...
- To temporarily allow direct network without Tor for local development:
  - Add Swift compiler flag: DOGECHAT_DEV_ALLOW_CLEARNET
